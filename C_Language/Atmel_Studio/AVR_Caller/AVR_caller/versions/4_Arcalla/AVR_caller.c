...
int main(void)
{
	...
	GIFR = 1<<INTF0;											// Сбросим флаги возможно возникавших ранее внешних прерываний
	GICR = 1<<INT0;												// Разрешим прерывания INT1 и INT0	
	#endif
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------			
		if (ppk_mode == ALARM_ACTIVE)							// Если возникла тревога
		{
			...
			#else
			OUT_PORT |= 1<<SIREN|1<<OUT_2|1<<OUT_1;				// Включим сирену, OUT_1 и и OUT_2
			#endif
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------
		...
		if ((ppk_mode & (1<<7)) && (simcom_init_mode == 1))		// Если была нажата ТРК, и модуль SIMCOM находиться в рабочем режиме, начинаем звонить
		{
			...
			GIFR |= 1<<INTF0;									// Сбросим флаги возникавших ранее прерываний
			GICR |= 1<<INT0;									// Разрешим прерывания INT1	
		}
		#endif			
	}
}
...
ISR (INT0_vect)													// Прерывание по INT0
{
	...
	GICR &= ~(1<<INT0);											// Запретим прерывания INT1
	#else
		...
		if (check_button_counter == 0)
		{
			...
			if (!(pin_state & (1<<BUTTON_PIN)))					// Если кнопка постановки/снятия нажата, активируем переход в другой режим
			{
				...
				{			
					...
					#ifdef USE_INT1_AS_TRK
					GIFR = 0<<INT0;	
					GICR = 0<<INT1|1<<INT0;						// Разрешим прерывания INT0 и запретим прерывание INT1
					#else
					GICR = 0<<INT1|0<<INT0;						// Запретим прерывания INT1 и INT0
					#endif
...
void Siren_Outs_OFF(void) 
{
	...
	{
		#if defined (DEBUG)
		OUT_PORT |= 1<<OUT_2|1<<OUT_1;							// ТОЛЬКО ДЛЯ ОТЛАДКИ
		#else			
		OUT_PORT &= ~(1<<OUT_2|1<<OUT_1);						// Выключим выходы OUT_1,			
		#endif
	}
}
//=====================================================================================================================================================
